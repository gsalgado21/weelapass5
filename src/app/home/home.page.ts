...
import { HERE_MAP_API_KEY} from '../../environments/environment';
import { Router } from '@angular/router';
import * as H2 from '@here/maps-api-for-javascript';
...
export class HomePage {
  ...
  constructor(public nav: NavController, public platform: Platform, public placeService: PlaceService, private diagnostic: Diagnostic,
    private geolocation: Geolocation, public utils: Utils, private api: ApiService, private alertController:AlertController,
  ...
  ionViewDidLoad() {
    ...
    this.api.getTripInProgress().subscribe(data => {
      if (data && data.result == 'success') {
        if (data.trip.status == 'DONE') this.nav.navigateRoot('SummaryPage');
        else this.nav.navigateRoot('TrackingPage');
      }
    });
    this.checkGPS();
  ...
  async choosePlace(attr) {
    const modal = await this.modalController.create({
      component: PlacesPage,
      componentProps: { type: attr }
    });
  ...
  private async loadCurrentPosition(map: any) {
    try {
        const resp = await this.geolocation.getCurrentPosition({ timeout: 5000, enableHighAccuracy: true, maximumAge: Number.MAX_SAFE_INTEGER } as any);
        const lat_lng = new H.geo.Point(resp.coords.latitude, resp.coords.longitude);
        ...
        const geocoder = this.here_api.getGeocodingService();
        ...
        geocoder.reverseGeocode(
            ...
            result => {
                ...
                this.trip.origin_vicinity = location.Location.Address.Label;
                this.trip.origin_latitude = location.Location.DisplayPosition.Latitude;
                this.trip.origin_longitude = location.Location.DisplayPosition.Longitude;
  ...
  private throwSimulationError() {
    this.utils.showAlert('Que Pena', 'Ocorreu um erro ao simular esta corrida. Tente Novamente mais tarde.', [{ text: 'Ok', handler: () => { this.nav.navigateRoot('HomePage') } }], false);
  }
  ...
  async requestCar() {
    ...
    try {
        const data = await this.api.requestTrip(this.trip).toPromise();

        if (data && data.result == 'success') {
            const popover = await this.utils.showPopover('HomePopover', null, { trip_id: data.trip_id });
            const { data: trip_status } = await popover.onWillDismiss();

            if (!trip_status) {
                const data_aux = await this.api.cancelTrip(data.trip_id).toPromise();
                if (data_aux && data_aux.result == 'success') {
                    this.nav.navigateForward('HomePage'); // Use navigateForward ou outro método conforme sua necessidade
                }
            } else if (trip_status == 'WAITING') {
                this.nav.navigateForward('TrackingPage'); // Use navigateForward ou outro método conforme sua necessidade
            } else if (trip_status == 'CANCELED') {
                await this.utils.showAlert(
                    'Que Pena', 
                    'Infelizmente não encontramos nenhum motorista apto a realizar sua corrida. Tente Novamente mais tarde.', 
                    [{ text: 'Ok', handler: () => { this.nav.navigateForward('HomePage') } }], 
                    false
                );
            }
        } else {
      ...
      inputs: [
        {
          name: 'coupon',
          type: 'text',
          placeholder: 'Informe o código para validação'
        }
      ],
      buttons: [
        {
